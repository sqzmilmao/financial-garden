...
export function GoalModal({ flowerId, isOpen, onClose, accentColor }: GoalModalProps) {
  ...
  const progressPercent = useMemo(() => Math.round((data?.goalProgress ?? 0) * 100), [data?.goalProgress])
        ...
        >
          ...
          {hasError && (
            <div className="flex h-96 flex-col items-center justify-center gap-4 text-center text-gray-700">
              <p>We couldn&rsquo;t load the details for this flower.</p>
            ...
            <div className="flex max-h-[80vh] flex-col gap-6 lg:flex-row">
              ...
              <div className="flex flex-[3] flex-col rounded-2xl border border-gray-100 bg-gray-50">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
                  {messages.length === 0 && (
                    <p className="text-center text-sm text-gray-500">No messages yet. Say hello to your assistant!</p>
                  )}
                    ...
                    <div key={`${message.id ?? idx}-${idx}`} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div
                        className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${
                          message.role === "user" ? "bg-[#2D9A86] text-white" : "bg-white text-gray-800 shadow-sm"
                        }`}
                      >
                        {message.content}
                      </div>
                ...
                <div className="border-t border-gray-200 p-4">
                  <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={handleSend} style={{ backgroundColor: "#2D9A86" }} className="rounded-full px-4 text-white">
